...
import threading
import time
from market import create_order,call_order
...
def iter_order(orders_by_price:dict):
  ...
  for price,quantity in orders_by_price.items():
    order_data = create_order(order_type="buy", price=price, quantity=quantity)
...
def iter_order_threaded(orders_by_price: dict):
    ...
    for price, quantity in orders_by_price.items():
        ...
        order_data = create_order(order_type="sell", price=price, quantity=quantity)  # Use absolute value for sell quantity
        thread = threading.Thread(target=call_order, args=(order_data,))
        thread.start()
        threads.append(thread)
